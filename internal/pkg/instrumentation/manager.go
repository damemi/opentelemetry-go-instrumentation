...
func NewManager(logger *slog.Logger, otelController *opentelemetry.Controller, probes []probe.BaseProbe, cp ConfigProvider, version string) (*Manager, error) {
	...
	for _, p := range probes {
		...
		case probe.GoLibraryTelemetryProbe:
			...
			for _, s := range probe.Manifest().Symbols {
			...
			for _, s := range probe.Manifest().Symbols {
				for _, d := range s.DependsOn {
					if _, exists := funcsMap[d]; !exists {
						return nil, fmt.Errorf("library %s has declared a dependent function %s for probe %s which does not exist, aborting", id, d, s.Symbol)
					}
...
// out of the Manager.
func (m *Manager) FilterUnusedProbes(target *process.TargetDetails) error {
	...
	for name, inst := range m.probes {
		...
		case probe.GoLibraryTelemetryProbe:
			...
			probe.TargetConfig().TargetDetails = target
			probe.TargetConfig().Executable = exe
		}
	}
	return err
}
...
func (m *Manager) applyConfig(c Config) error {
	...
	for id, p := range m.probes {
		if telemetryProbe, ok := p.(probe.RunnableProbe); ok {
			...
			if currentlyEnabled && !newEnabled {
				...
				err = errors.Join(err, telemetryProbe.Close())
				continue
			}
			...
			if !currentlyEnabled && newEnabled {
				...
				err = errors.Join(err, telemetryProbe.Load())
				if err == nil {
...
func (m *Manager) cleanup(target *process.TargetDetails) error {
	...
	for _, i := range m.probes {
		if probe, ok := i.(probe.RunnableProbe); ok {
