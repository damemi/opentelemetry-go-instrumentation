...
type BaseProbe interface {
	...
	// config formats it defines.
	ApplyConfig(Config) error
}

// RunnableProbe is a Probe that processes Telemetry events.
// It is a BaseProbe that runs continuously, passing Telemetry to
// a handler function (such as a Collector pipeline).
type RunnableProbe interface {
	BaseProbe

	// Run starts the Probe. It accepts a handler function that accepts Telemetry
	// data processed by the Probe.
	Run()
...
// TracingProbe is a TelemetryProbe that handles Traces.
...
// GoLibraryTelemetryProbe is a TelemetryProbe that processes Telemetry
// events from a single Go library in a single observed application.
